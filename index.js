...
class GlobalAudioManager {
    ...
    bindDOMElements() {
        this.els = {
            ...
            // ノイズキャリブレーション
            noiseCalBtn: document.getElementById('noiseCalibrationBtn'),
            noiseStatus: document.getElementById('noiseStatusIndicator'),
            // 楽器選択
    ...
    setupEventListeners() {
        ...
        // === ノイズキャリブレーション ===
        this.els.noiseCalBtn?.addEventListener('click', () => this.startNoiseCalibration());

        // === 楽器選択 ===
        this.els.instrumentSelect?.addEventListener('change', (e) => {
    ...
    setPracticeMode(mode) {
        ...
        // === 新しいピッチ検出システムにモードを通知 ===
        if (window.pitchEngine && window.pitchEngine.setMode) {
            // 'solo' -> 'precision', 'ensemble' -> 'robust'
            const pitchMode = mode === 'ensemble' ? 'robust' : 'precision';
            window.pitchEngine.setMode(pitchMode);
            console.log(`PitchEngine mode set to: ${pitchMode}`);
        }
    ...
    async initNoiseReduction() {
        // NoiseReductionManagerがない場合はスキップ
        if (!window.NoiseReductionManager) {
            console.log('NoiseReductionManager not available');
            return;
        }

        const self = this;

        this.noiseReductionManager = new NoiseReductionManager({});

        // コールバックを直接設定（内部プロパティにアクセス）
        this.noiseReductionManager._onCalibrationStart = () => {
            console.log('[Noise] Calibration started');
            self.updateNoiseUI('calibrating');
        };

        this.noiseReductionManager._onCalibrationProgress = (progress) => {
            console.log(`[Noise] Progress: ${Math.round(progress * 100)}%`);
            const btn = self.els.noiseCalBtn;
            const status = self.els.noiseStatus;
            if (btn) {
        ...
        this.noiseReductionManager._onCalibrationComplete = (numFrames) => {
            console.log(`[Noise] Calibration complete: ${numFrames} frames`);
            self.updateNoiseUI('active');
        };

        this.noiseReductionManager._onCalibrationFailed = (reason) => {
            console.error('[Noise] Calibration failed:', reason);
            self.updateNoiseUI('off');
        };

        // AudioContextが準備できたら初期化
        if (this.audioContext) {
            await this.noiseReductionManager.initialize(this.audioContext);
        }

        console.log('NoiseReductionManager created');
    }

    async startNoiseCalibration() {
        console.log('[NoiseCalibration] Starting...');
        ...
        // NoiseReductionManagerを初期化
        if (!this.noiseReductionManager) {
            await this.initNoiseReduction();
        }

        if (!this.noiseReductionManager) {
            console.error('[NoiseCalibration] NoiseReductionManager not available');
            this.updateNoiseUI('off');
            return;
        }

        // AudioContextを渡して初期化
        if (!this.noiseReductionManager._isInitialized) {
            console.log('[NoiseCalibration] Initializing NoiseReductionManager...');
            const initialized = await this.noiseReductionManager.initialize(this.audioContext);
            if (!initialized) {
                console.error('[NoiseCalibration] Failed to initialize NoiseReductionManager');
                this.updateNoiseUI('off');
                return;
            }
        }

        // 専用マイクストリームを取得（キャリブレーション用）
        try {
            console.log('[NoiseCalibration] Getting microphone stream...');
            ...
            // 専用のMediaStreamSource作成
            this._noiseCalibrationSource = this.audioContext.createMediaStreamSource(stream);
            this._noiseCalibrationStream = stream;

            // WorkletノードをMicに接続
            const noiseNode = this.noiseReductionManager.getNode();
            if (noiseNode) {
                this._noiseCalibrationSource.connect(noiseNode);
                // 出力先（スピーカーに出す必要はないので、サイレントに）
                // ただしAudioWorkletはprocessが呼ばれるためにどこかに接続が必要
                noiseNode.connect(this.audioContext.destination);
                console.log('[NoiseCalibration] Connected: mic → noiseNode → destination');
            }

            // UI更新
            this.updateNoiseUI('calibrating');

            // キャリブレーション開始
            console.log('[NoiseCalibration] Sending startCalibration command...');
            this.noiseReductionManager.startCalibration();

        } catch (error) {
            console.error('[NoiseCalibration] Microphone access failed:', error);
            this.updateNoiseUI('off');
        }
    }

    updateNoiseUI(state) {
        const btn = this.els.noiseCalBtn;
        const status = this.els.noiseStatus;

        if (!btn || !status) return;

        btn.classList.remove('calibrating', 'active');
    ...
    setInstrument(instrumentType) {
        ...
        console.log(`🎵 Instrument set to: ${instrumentType}`);

        // UnifiedPitchEngineに反映
        if (window.pitchEngine && window.pitchEngine.setInstrument) {
            window.pitchEngine.setInstrument(instrumentType);
        }

        // グローバルイベントを発火（モジュールに通知）
        window.dispatchEvent(new CustomEvent('suiren-instrument-changed', {
            detail: { instrument: instrumentType }
        }));
    }
    ...
    updateSourceRouting() {
        ...
        // === 新しいAudioPipeline使用 ===
        if (this.usePipeline && this.pipeline) {
            this.pipeline.setSourceType(this.isFileMode ? 'file' : 'mic');
            return; // Pipelineがルーティングを管理
        }
        ...
        if (this.isFileMode) {
            // ファイルモード - プレーヤーのみをAnalyserに接続
            try {
                this.playerSource?.connect(this.analyser);
                this.analyser.connect(this.audioContext.destination);
            } catch (e) { }
        } else {
            // マイクモード - マイクのみをAnalyserに接続
            ...
            // マイクモードでも音源を聴けるように、playerSourceはdestinationに直接接続
            try {
    ...
    saveSettings() {
        const settings = {
            ...
            instrument: this.els.instrumentSelect?.value || 'default'
        };
    ...
    restoreSettings() {
        ...
        try {
            ...
            // 楽器選択を復元
            if (settings.instrument && this.els.instrumentSelect) {
                this.els.instrumentSelect.value = settings.instrument;
                this.setInstrument(settings.instrument);
            }
        } catch (e) {
...
class TreeLayoutManager {
    ...
    renderModuleCards() {
      ...
      `;
            ...
            // ドラッグイベント設定（マウス） - Custom Drag & Dropに統一
    ...
    /**
     * モジュールカードのマウスダウン（ドラッグ開始）
     */
    onCardMouseDown(e, card) {
        if (e.button !== 0) return; // 左クリックのみ
        e.preventDefault(); // テキスト選択などを防止

        this.startDrag(card, e.clientX, e.clientY, 'mouse');
    }
    ...
    onCardTouchStart(e, card) {
        ...
        this.touchDragState.timer = setTimeout(() => {
            if (this.touchDragState && this.touchDragState.card === card) {
                this.startDrag(card, touch.clientX, touch.clientY, 'touch');
            }
        }, 200);
    ...
    setupWorkspaceDropEvents() {
        this.dropZones.querySelectorAll('.drop-zone').forEach(zone => {
            zone.addEventListener('dragover', (e) => {
                ...
                // ドロップゾーンへのホバーを記録
                this.currentHoverZone = { zone: zone.dataset.zone };
            });
...
// ===== UnifiedPitchEngine初期化 =====
document.addEventListener('DOMContentLoaded', async () => {
    if (window.UnifiedPitchEngine) {
        window.pitchEngine = new UnifiedPitchEngine({
            a4: 442,
            mode: 'precision',
            instrument: 'default'
        });
        ...
        try {
            await window.pitchEngine.initialize();
            console.log('🎵 UnifiedPitchEngine initialized:', window.pitchEngine.getStatus());
        } catch (e) {
            console.error('❌ UnifiedPitchEngine initialization failed:', e);
        }
    } else {
        console.warn('⚠️ UnifiedPitchEngine not found');
    }
});
