...
class TunerModule {
  ...
  processLoop() {
    ...
    if (window.pitchEngine) {
      ...
      const pitchResult = window.pitchEngine.analyze(this.timeBuf, sr, this.freqBuf);
      ...
      if (pitchResult && pitchResult.freq && pitchResult.confidence > 0.3) {
        ...
        if (algoResults) {
          this.rawAlgoFreqs = {
            yin: algoResults.yin?.freq || 0,
            mpm: algoResults.mpm?.freq || 0,
            fft: algoResults.fft?.freq || 0
          };
          this.perceivedFreq = algoResults.perceived?.freq || pitchResult.freq;
        } else {
          this.rawAlgoFreqs = {
        ...
        if (pitchResult.prediction) {
          this.predictionDisplay = {
            predictedFreq: pitchResult.prediction.predictedFreq || 0,
        ...
        // === 外れ値除去処理 ===
        let finalFreq = pitchResult.freq;
        finalFreq = this.applyOutlierRejection(finalFreq, pitchResult.confidence);

        if (this.currentStableFreq > 0) {
          const diffRatio = finalFreq / this.currentStableFreq;
          if (diffRatio > 1.06 || diffRatio < 0.94) {
            this.resetSwayHistory();
          }
        }

        this.currentStableFreq = finalFreq;
        this.updateUpperUI(finalFreq, pitchResult.confidence);

        // Update History - 各アルゴリズムの実際の値を使用
        this.updateSwayHistory('yin', this.rawAlgoFreqs.yin);
        this.updateSwayHistory('mpm', this.rawAlgoFreqs.mpm);
        this.updateSwayHistory('fft', this.rawAlgoFreqs.fft);
        this.updateSwayHistory('perc', this.perceivedFreq);
      } else {
        // 信頼度が低い場合はリセット
        this.resetState();
      }
  ...
  /**
   * 外れ値除去処理
   * 急激なピッチ変化を検出し、信頼できる値のみを返す
   */
  applyOutlierRejection(freq, confidence) {
