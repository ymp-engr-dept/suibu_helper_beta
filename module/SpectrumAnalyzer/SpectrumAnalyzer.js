...
class SpectrumAnalyzer {
  ...
  draw() {
    ...
    // ページ非表示時のみスキップ（低電力モードでも描画は継続）
    if (pm && pm.shouldSkipDrawing('spectrum')) {
      return;
    }
    ...
    // ピーク周波数のマーカーを描画
    if (maxVal > 30 && maxBinIndex !== -1) {
      const peakFreq = maxBinIndex * freqPerBin;
      const peakX = this.freqToX(peakFreq);
      const peakY = this.els.canvas.height * (1 - (maxVal / 255) * gainVal);

      // ピークマーカー（小さな円）
      this.els.ctx.beginPath();
      this.els.ctx.fillStyle = 'rgba(255, 100, 100, 0.8)';
      this.els.ctx.arc(peakX, peakY, 4, 0, Math.PI * 2);
      this.els.ctx.fill();
    }

    // 基本周波数検出（PitchEngine統合・改善版）
    if (!isFrozen) {
      ...
      if (window.pitchEngine) {
        // リアルタイムピッチを取得
        const pitchResult = window.pitchEngine.getRealtimePitch();
        if (pitchResult && pitchResult.freq) {
          pitchConfidence = pitchResult.confidence;
          pitchFreq = pitchResult.freq;
        }
      }

      // 倍音表示の条件判定
      // UnifiedPitchEngineの閾値変更に合わせて調整 (0.5 -> 0.2)
      if (pitchConfidence > 0.2) {
        // 信頼度が高い → PitchEngineの周波数を使用
        this.detectedFundamentalFreq = pitchFreq;
        this._harmonicsSource = 'pitch';
      } else if (maxVal > 50 && maxBinIndex !== -1) {
        // 信頼度が低いが音量がある → FFTピークを使用
        this.detectedFundamentalFreq = maxBinIndex * freqPerBin;
        this._harmonicsSource = 'fft';
      } else {
        // 音量が小さいか音程がない → 倍音非表示
        this.detectedFundamentalFreq = null;
        this._harmonicsSource = null;
      }
    }

    // 倍音描画（条件付き）
    if (this.els.showHarmonics.checked && this.detectedFundamentalFreq) {
      this.drawHarmonicsLines(this.detectedFundamentalFreq, this._harmonicsSource);
    }
  ...
  drawHarmonicsLines(f0, source = 'pitch') {
    if (!f0 || f0 < this.MIN_FREQ) return;
    const ctx = this.els.ctx;
    const h = this.els.canvas.height;

    // ソースに応じた色設定
    const isPitchBased = source === 'pitch';
    const baseColor = isPitchBased ? 'rgba(0, 230, 118, 0.9)' : 'rgba(255, 165, 0, 0.7)';
    const baseLabel = isPitchBased ? '基音' : 'Peak';
    const overtoneColor = isPitchBased ? 'rgba(0, 200, 100, 0.5)' : 'rgba(255, 255, 0, 0.4)';
    ...
    if (x0 >= 0) {
      ...
      const labelWidth = ctx.measureText(baseLabel).width + 8;
      ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
      ctx.fillRect(x0 + 2, 15, labelWidth, 16);
      ctx.fillStyle = baseColor;
      ctx.fillText(baseLabel, x0 + 6, 27);

      // 周波数表示
      ctx.font = '10px sans-serif';
      ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
      ctx.fillText(`${Math.round(f0)}Hz`, x0 + 6, 42);
    }

    // 倍音ライン
    ctx.lineWidth = 1;
    ctx.strokeStyle = overtoneColor;
    ctx.fillStyle = overtoneColor;
    ...
    for (let n = 2; n <= 16; n++) {
      ...
      if (xn >= 0) {
        // グラデーション効果（高い倍音ほど薄く）
        const alpha = Math.max(0.1, 0.5 - (n - 2) * 0.03);
        ctx.strokeStyle = isPitchBased
