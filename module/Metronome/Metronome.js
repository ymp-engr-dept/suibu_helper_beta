...
class MetronomeModule {
    ...
    playCountInClick(time) {
        ...
        masterGain.gain.value = 0.5 * this.volume; // 準備カウントは控えめ + ユーザー音量
    ...
    playClick(time, isDownbeat, isBeatStart) {
        ...
        // === マスターゲイン ===
        const masterGain = this.audioContext.createGain();
        masterGain.gain.value = 0.7 * this.volume; // ユーザー音量適用
        masterGain.connect(this.audioContext.destination);
    ...
    updateStartButton() {
        ...
        const icon = this.els.startBtn.querySelector('.btn-icon');
        const text = this.els.startBtn.querySelector('.btn-text');

        if (this.isPlaying || this.isCountingIn) {
            this.els.startBtn.classList.add('playing');
            if (icon) icon.textContent = '❚❚';
            if (text) text.textContent = 'ストップ';
        } else {
            this.els.startBtn.classList.remove('playing');
            if (icon) icon.textContent = '▶';
            if (text) text.textContent = 'スタート';
        }
    }
}
