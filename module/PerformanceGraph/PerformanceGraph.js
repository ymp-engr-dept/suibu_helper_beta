...
class PerformanceGraphModule {
    ...
    async processLoop() {
        ...
        // 音量が閾値を超えている場合のみ音程解析
        if (rms >= minRMS && window.pitchEngine) {
            // Graph用に加工されたピッチを取得 (Short Note Rejection済み)
            // Short Note RejectionはPitchEngine側で処理されるため、ここでは結果を受け取るだけ

            // 注意: PitchEngineの解析自体はTunerまたはMainLoopで呼ばれている前提だが、
            // ここでも念のため呼ぶか、あるいは「最後に処理された結果」を取得するか？
            // Tunerが表示されていない場合、PitchEngineが動かない可能性がある。
            // そのため、ここで analyze を呼ぶのが安全。
            // PitchEngineは内部で同一フレームの重複解析を防ぐガードを持っているとベストだが、
            // 現状は持っていない。
            // しかし、WebAudioデータの取得(getFloatTimeDomainData)は同じバッファを返すので、
            // 複数回呼んでも結果は同じ（負荷だけかかる）。
            // PitchEngineに「前回解析時刻」があればスキップできる。
            // 今回は安全のためここで呼ぶ（Tuner非表示時も動くように）。

            window.pitchEngine.setA4(this.getA4());
            // 正確なタイムスタンプを渡すことで、Onset検出とグラフのタイミングを同期させる
            window.pitchEngine.analyze(this.timeBuf, sampleRate, this.freqBuf, audioDataTimestamp);

            const processed = window.pitchEngine.getProcessedPitch('graph');

            if (processed && processed.freq > 0) {
                freq = processed.freq;
                cents = processed.cents;

                // 予測データ
                if (processed.prediction) {
                    this._currentPredFreq = processed.prediction.freq || 0;
                    if (this._currentPredFreq > 0) {
                        const pInfo = this.getPitchInfo(this._currentPredFreq);
                        this._currentPredCents = pInfo.cents;
                    } else {
        ...
        this.pushDataPoint(audioDataTimestamp, rms, freq, cents, this._currentPredFreq, this._currentPredCents);

        // ===== 遡及描画：過去データの更新 =====
        if (window.pitchEngine && window.pitchEngine.getRetroactiveData) {
            const retroactiveData = window.pitchEngine.getRetroactiveData();
            if (retroactiveData && retroactiveData.length > 0) {
                this._updateRetroactiveData(retroactiveData);
            }
        }
    }

    // 遡及データで過去の履歴を更新
    _updateRetroactiveData(retroData) {
        // 効率化のため、履歴を後ろから探索
        // retroDataは時系列順になっている前提
        for (const update of retroData) {
            const targetTime = update.onsetTimestamp;

            // 履歴から最も近いタイムスタンプを探す（線形探索だが、履歴の末尾に近いので高速）
            // 許容誤差: 20ms (1フレーム分程度)
            for (let i = this.dataHistory.length - 1; i >= 0; i--) {
                const point = this.dataHistory[i];
                if (Math.abs(point.timestamp - targetTime) < 20) {
                    // 更新が必要な場合のみ更新
                    if (point.freq === 0 && update.freq > 0) {
                        point.freq = update.freq;
                        point.cents = update.cents;
                        // 色などを再計算する必要がある場合は描画時に行われる
                    }
                    break;
                }

                // ターゲットより新しすぎる場合はスキップ、古すぎる場合は探索終了
                if (point.timestamp < targetTime - 50) {
                    break;
                }
            }
        }
    ...
    drawMetronomeBars(ctx, w, h, currentX) {
        ...
        // 1. 未来方向（左側）へ描画
        // 現在時刻より後にある拍を探す
        // timeToNextBeat は 0〜secondsPerBeat の値
        let futureBeatTime = timeToNextBeat;
        let protection = 0;

        while (protection < 200) {
            if (futureBeatTime > futureDisplaySeconds) break; // 画面左端を超えた

            const x = currentX - futureBeatTime * pixelsPerSecond;
            ...
            futureBeatTime += secondsPerBeat;
            protection++;
        }

        // 2. 過去方向（右側）へ描画
        // 現在時刻より前にある拍を探す
        // 直近の過去の拍は futureBeatTime - secondsPerBeat だが、timeToNextBeat - secondsPerBeat で計算可能
        // timeToNextBeatは「次の拍」までの時間なので、
        // 直前の拍は (timeToNextBeat - secondsPerBeat) つまり負の値
        let pastBeatTime = timeToNextBeat - secondsPerBeat;
        protection = 0;

        const maxPastTime = -totalDisplaySeconds; // 表示限界（負の値）

        while (protection < 200) {
            // pastBeatTime は負の値。絶対値が表示秒数を超えたら終了
            if (pastBeatTime < maxPastTime) break;

            // x座標計算: currentX から、経過時間分だけ右へ
            // pastBeatTimeは負なので、マイナスしてプラスにする
            const x = currentX - pastBeatTime * pixelsPerSecond;
