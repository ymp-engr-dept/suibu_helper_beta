...
class AudioPipeline {
    ...
    /**
     * ファイル入力を設定
     * 注: Analyserへの接続はsetSourceType()で制御する
     */
    setFileSource(audioElement) {
        ...
        try {
            ...
            // 初期状態ではdestinationのみに接続（マイクモードがデフォルト）
            this.playerSource.connect(this.gainNode);
            this.gainNode.connect(this.audioContext.destination);

            // sourceTypeは変更しない（setSourceType()で制御）
            return true;
        } catch (e) {
    ...
    /**
     * ソースタイプを切り替え（ファイル/マイク）
     * ファイルモード時はマイクをAnalyserから切断
     * マイクモード時はファイルをAnalyserから切断
     */
    setSourceType(type) {
        if (type !== 'mic' && type !== 'file') return;
        if (type === this.sourceType) return;

        this.sourceType = type;

        if (type === 'file') {
            // ファイルモード: マイク → Analyser 接続を切断
            this._disconnectMicFromAnalyser();
            this._connectFileToAnalyser();
            this._emitStatus('file-mode', 'File Mode');
        } else {
            // マイクモード: ファイル → Analyser 接続を切断、ファイルはdestinationのみ
            this._disconnectFileFromAnalyser();
            this._connectMicRouting();
            // マイクモードでもファイルを聴けるようにdestinationに接続
            this._connectFileToDestinationOnly();
            this._emitStatus(this.currentMode === 'ensemble' ? 'ensemble-mode' : 'mic-ready',
                this.currentMode === 'ensemble' ? '合奏モード' : 'Mic Mode');
        }
    }

    /**
     * マイクからAnalyserへの接続を切断
     */
    _disconnectMicFromAnalyser() {
        try {
     ...
     * ファイルをAnalyserに接続
     */
    _connectFileToAnalyser() {
        if (!this.playerSource || !this.gainNode || !this.analyser) return;
        try {
            this.playerSource.disconnect();
    ...
    /**
     * ファイルをdestinationのみに接続（Analyserには接続しない）
     */
    _connectFileToDestinationOnly() {
        if (!this.playerSource || !this.gainNode) return;
        try {
            this.playerSource.disconnect();
            this.gainNode.disconnect();
            this.playerSource.connect(this.gainNode);
